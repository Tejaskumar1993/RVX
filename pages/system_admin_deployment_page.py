...
class SystemAdminDeploymentsPage:
    ...
    def __init__(self, page: Page):
        ...
        self.deployment_summary_tab_navigation = '//button[text()="<<tab_to_navigate>>"]'
        ...
        self.deployment_delete_success_message = page.locator('//div[@class="ant-message-custom-content ant-message-success"]')
        ...
        self.deployment_summary_title = page.locator('//div[text()="Deployment Summary"]')
        self.deployment_summary_users_tab = page.locator('//button[text()="Users"]')
        self.deployment_summary_published_items_tab = page.locator('//button[text()="Published Items"]')
        self.deployment_summary_sends_tab = page.locator('//button[text()="Sends"]')
        self.deployment_summary_account_balance_tab = page.locator('//button[text()="Account Balance"]')
        self.deployment_summary_information_tab = page.locator('//button[text()="Information"]')
        self.deployment_summary_users_filter = page.locator('//div[@class="deployment-summary-content"]//label[text()="Filter:"]')
        self.deployment_summary_user_filter_dropdown = page.locator('//div[@class="deployment-summary-content"]//div[@class="card-body"]//select')
        ...
        self.user_action_button = page.locator('(//button[@class="btn rounded-3 me-2 fs--2 action-item-sm btn-outline icon-item icon-item-md"])[1]')
        self.multi_actions_button = page.locator('(//div[@id="order-information-tab-tabpane-users"]//div[@class="font-sans-serif btn-reveal-trigger dropend"])[1]')
        self.assign_group_button = page.locator('//a[text()="Assign Group"]')
        self.success_message = page.locator('//div[@class="ant-message-custom-content ant-message-success"]')
        self.group_filter = page.locator('(//div[@class="modal-body"]//label)[2]')
        self.group_filter_options = page.locator('((//div[@class="modal-content"])[2]//select)[1]')
        ...
        self.active_user_button = page.locator('//button[text()="Activate User" or text()="Suspend User"]')
        self.archive_user_button = page.locator('//button[text()="Archive User" or text()="Unarchive User"]')
        ...
        self.published_items_tab_header = page.locator('(//tr[@class="text-center"])[3]')
        self.published_items_name = page.locator('(//th[text()="Name"])[2]')
        self.published_items_description = page.locator('(//th[text()="Description"])[1]')
        # Sends tab locators
        ...
        self.accountbalance_balance = page.locator('//div[@class="pt-4 pb-3 card-body"]')

    @qase_screenshot
    @qase.step(
        title="Verify and click on Deployments tab",
        expected="Verify Users tab icon and click on deployments tab",
    )
    def verify_and_click_on_deployments_tab(self, side_navigation_item):
        ...
        self.page.click(self.deployments_tab.replace("<<side_navigation_tabs>>", side_navigation_item))
    ...
    def verify_deployments_page_elements(self, headers_text):
        ...
        cleaned_headers_title = re.sub(r'[🔼🔽]', '', headers_title).strip()  # Remove sorting icons
        # Split the headers on tabs or multiple spaces
        headers_list = re.split(r'[\t]+|\s{2,}', cleaned_headers_title)
        headers_list = [header.strip() for header in headers_list if header]  # Remove empty items
        # Assert the headers match the expected values
        assert headers_list == headers_text, f"Headers do not match: {headers_list} != {headers_text}"
    ...
    def verify_deployment_summary_users_filter(self, available_filter_options):
        ...
        filter_options = self.page.query_selector_all(self.deployment_summary_user_dropdown_options)
    ...
    def apply_filter_on_deployment_summary_users_data(self, available_filter_options, expected_statuses):
        """
    ...
    def verify_deployment_summary_published_items_tab_fields(self, headers_text, tab_to_navigate):
        """
        ...
        self.select_deployment.click()
        self.page.wait_for_load_state('domcontentloaded')
        self.page.click(self.deployment_summary_tab_navigation.replace("<<tab_to_navigate>>", tab_to_navigate))
    ...
    def verify_deployment_summary_account_balance_tab_fields(self, account_balance_title, tab_to_navigate):
        """
    ...
    def verify_actions_button_on_users_tab(self, success_message, available_filter_options, expected_statuses, error_message):
        """
        ...
        if self.success_message.is_visible():
            ...
            if message_text == success_message:
                ...
                assert message_text == success_message, f"Expected success message '{success_message}', but got '{message_text}'"
            elif message_text == error_message:
                print("Error message verified.")
                assert message_text == error_message, f"Expected error message '{error_message}', but got '{message_text}'"
            else:
